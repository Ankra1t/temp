...
handler_fileout = TimedRotatingFileHandler(
    filename=os.path.join("logs", "main_logs.log"),
    when='W6',
    backupCount=10,
    encoding='utf-8'
)
...
handler_fileout_sendlog.setFormatter(
    Formatter(fmt='[%(asctime)s: %(levelname)s] %(message)s'))
...
handler_fileout_no_send = TimedRotatingFileHandler(
    filename=os.path.join("logs/sendlog", "log_send_no_send.log"), when='H', interval=8,
    backupCount=12, encoding='utf-8')
handler_fileout_no_send.setFormatter(
    Formatter(fmt='[%(asctime)s: %(levelname)s] %(message)s'))
...
handler_fileout_send_ok = TimedRotatingFileHandler(filename=os.path.join("logs/sendlog", "log_send_send_ok.log"), when='H', interval=8,
                                                   backupCount=12, encoding='utf-8')
handler_fileout_send_ok.setFormatter(
    Formatter(fmt='[%(asctime)s: %(levelname)s] %(message)s'))
